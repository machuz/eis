...
**構造的影響** — システムの形がそのエンジニアにどれだけ依存しているか — を測定する別の複合シグナル。良いアーキテクトは「**生き残り**、かつ他者が**その上に積み上げる（catalysis）**」コードを書く。よって重力は両者を**必要条件**として扱う：構造的**シェイプ**を 2 つのゲートに通す。
...
シェイプ（設計・到達範囲・単独所有）は「何を形作ったか」だが、各軸は変更*履歴*から算出されるため、**後で書き換えられた土台でもシェイプは高いまま残る**。2 つのゲートがこれを補正する。**survGate**：そのコードは生き残ったか？ Survival は総時間減衰生存なので、静かなモジュールに安定化した創始者のコードは通る — だが書き換えられて消えた「土台」（Survival → 0）は、どれだけ設計履歴を積んでいても下限に落ちる。**重力はコードとともに減衰する**。構造依存とは「今そこにあるコード」への依存だからだ。**catGate**：それは関係的か？ Catalysis（他者が自分の生存コードの上に積む）だけが一人プロジェクトで必然ゼロになり、構造がそのエンジニアに寄りかかっている唯一の証人となる。一人プロジェクトは重力を捏造できない。ここで RobustSurvival は使わない — 創始者のコードが低変更圧モジュールに安定化すると robust はほぼ 0 になるため、総 Survival こそが「まだ荷重を支えているか」の誠実な信号である。
