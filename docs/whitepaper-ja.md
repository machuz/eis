...
**構造的影響** — システムの形がそのエンジニアにどれだけ依存しているか — を測定する別の複合シグナル。重力は*関係的*であり、一人だけでは観測できない。そのため全軸を線形に足すのではなく、構造的**フットプリント**を**関係ゲート**で増幅する形を取る。
...
フットプリントの各軸（生存・設計・到達範囲・単独所有）はいずれも単独の作者が到達可能で、それ自体は*到達範囲*を表すに過ぎず重力ではない。Catalysis（他者が自分の生存コードの上に積み上げること）だけが一人プロジェクトで必然的にゼロになり、構造がそのエンジニアに寄りかかっていることの唯一の証人となる。ゆえに Catalysis は加算項ではなくフットプリントを**ゲート**する。誰も積み上げていなければゲートは下限（0.2）に落ち、フットプリントが大きくても一人プロジェクトは約20で頭打ちになる。一方フットプリントの重みは、solo抑制を兼ねさせる必要がなくなり、構造的重要度を素直に序列化できる（RobustSurvival 主軸）。フットプリントの生存項は **RobustSurvival** のみ — DormantSurvival（静かな低変更圧モジュールに眠るコード）は耐久的だが他者が積み上げる引力を生まないため除外し、変更圧データが無い場合は総 Survival を強く割り引いてのみ計上する。
