...
**The screens, operations, and micro-interactions users touch directly.**

Observation targets:
- Per-screen behavior, render performance, interaction quality
- Bug reports, customer support tickets
- Heatmaps, click-through rates, error rates
- "Is it usable / not usable" signals at the screen level

Working axis: **does it work, can it be used.** Same question as the organization's Implementation layer.
...
Working axis: **does the product hang together as a whole.** As with the organization's Intermediate layer, **translation lives here** — aligning intent (Principle) with screens (Implementation).
...
Working axis: **can someone explain what this decision is for.** Because AI can't make Principle-layer calls, the human decision history needs to be visible — same as the organization's Principle layer.
