...
def write_markdown_report(analyses: list[RepoAnalysis],
    ...
    with open(out, "w") as f:
        ...
        f.write("sapphi-red (Vite), Jose Valim (Phoenix), fisker Cheung (Prettier), Taylor Otwell (Laravel), ")
        f.write("Josh Goldberg (ESLint), Donny / kdy1 (swc), Ritchie Vink (Polars): their gravity saturates their universe. ")
        ...
        f.write("The gates also stay quiet where founding alone would have flattered. A foundation no one else ")
        f.write("still presses on reads as settled, not central: the telescope cannot see a structure no one is ")
        f.write("currently leaning on under load. Sebastian Markbåge sits at the top of React's gravity with the ")
        f.write("shape saturated — Indispensability 100, others plainly built on him (Catalysis 100) — yet his ")
        f.write("gravity reads a modest 10.8, because the Reconciler he laid down now sits largely undisturbed: ")
        f.write("robust survival near zero, no live pressure from others to witness. That is not a demotion. ")
        f.write("It is the instrument declining to confuse ownership, or a finished foundation, with gravity.\n\n")
