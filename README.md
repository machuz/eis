...
- **Vendored & generated code doesn't count** — `vendor/`, `third_party/`, and `node_modules/` trees, dependency lockfiles, and generated files (`*.pb.go`, `*_pb2.py`, minified bundles) are excluded by default, plus anything marked `linguist-generated`/`linguist-vendored` in `.gitattributes`. So committing third-party code can't inflate anyone's signal — and it keeps blame off tens of thousands of files it shouldn't read (a vendored Go monorepo is mostly `vendor/`). Add your own patterns via `exclude_file_patterns` in `eis.yaml`; a trailing `/` excludes a whole directory at any depth.
